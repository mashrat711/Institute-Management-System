...
class TCPDF {
	...
	/**
	 * ID of the stored default header files (-1 = not set).
	 * @protected
	 */
	protected $header_xobjid = false;

	/**
	 * If true reset the Header Xobject files at each page
	 * @protected
	 */
	 ...
	 * @param $layout (string) The page Backend. Possible values are:<ul><li>SinglePage Display one page at a time</li><li>OneColumn Display the pages in one column</li><li>TwoColumnLeft Display the pages in two columns, with odd-numbered pages on the left</li><li>TwoColumnRight Display the pages in two columns, with odd-numbered pages on the right</li><li>TwoPageLeft (PDF 1.5) Display the pages two at a time, with odd-numbered pages on the left</li><li>TwoPageRight (PDF 1.5) Display the pages two at a time, with odd-numbered pages on the right</li></ul>
	...
	public function AddPage($orientation='', $format='', $keepmargins=false, $tocpage=false) {
		if ($this->inxobj) {
			// we are inside an XObject files
			return;
		}
	...
	/**
	 * Reset the xobject files used by Header() method.
	 * @public
	 */
	...
	/**
	 * Set a flag to automatically reset the xobject files used by Header() method at each page.
	 * @param $val (boolean) set to true to reset Header xobject files at each page, false otherwise.
	 * @public
	 */
	...
	public function Header() {
		...
		if ($this->header_xobj_autoreset) {
			// reset header xobject files at each page
			$this->header_xobjid = false;
		}
	}
	...
	public function Annotation($x, $y, $w, $h, $text, $opt=array('Subtype'=>'Text'), $spaces=0) {
		if ($this->inxobj) {
			// store parameters for later use on files
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	public function addHTMLTOC($page='', $toc_name='TOC', $templates=array(), $correct_align=true, $style='', $color=array(0,0,0)) {
		...
		$page_fill_start = false;
		$page_fill_end = false;
		// get the font type used for numbers in each files
	...
	*/
	...
	public function startTemplate($w=0, $h=0, $group=false) {
		if ($this->inxobj) {
			// we are already inside an XObject files
			return false;
		}
		$this->inxobj = true;
		++$this->n;
	 ...
	 * @param $align (string) Indicates the alignment of the pointer next to files insertion relative to files height. The value can be:<ul><li>T: top-right for LTR or top-left for RTL</li><li>M: middle-right for LTR or middle-left for RTL</li><li>B: bottom-right for LTR or bottom-left for RTL</li><li>N: next line</li></ul>
	 * @param $palign (string) Allows to center or align the files on the current line. Possible values are:<ul><li>L : left align</li><li>C : center</li><li>R : right align</li><li>'' : empty string : left for LTR or right for RTL</li></ul>
	 * @param $fitonpage (boolean) If true the files is resized to not exceed page dimensions.
	 * @author Nicola Asuni
	 * @public
	 * @since 5.8.017 (2010-08-24)
	...
	public function printTemplate($id, $x='', $y='', $w=0, $h=0, $align='', $palign='', $fitonpage=false) {
		...
		if ($this->inxobj) {
			if ($id == $this->xobjid) {
				...
				// use the files as resource for the files currently opened
		...
		// calculate files width and height on document
		if (($w <= 0) AND ($h <= 0)) {
			$w = $ow;
			$h = $oh;
		} elseif ($w <= 0) {
		...
		// fit the files on available space
		list($w, $h, $x, $y) = $this->fitBlock($w, $h, $x, $y, $fitonpage);
		// set page alignment
		$rb_y = $y + $h;
		// set alignment
		...
		switch($name) {
			...
			case 'tspan': {
				if ($this->svgtextmode['invisible']) {
					...
					// If the 'visibility' property is set to hidden on a 'tspan', 'tref' or 'altGlyph' element, then the text is invisible but still takes up space in text Backend calculations.
					break;
				}
				// print text
				$text = $this->svgtext;
